    ...
    companion object {
        fun buildVODStreamsObjectRequest(favStreamers: List<String>):JSONArray {
            ...
            for (channelOwnerLogin in favStreamers) {
                ...
                variablesObject.put("includePreviewBlur", false)
                variablesObject.put("channelOwnerLogin", channelOwnerLogin)
                variablesObject.put("limit", 30)
                variablesObject.put("broadcastType", "ARCHIVE")
        ...
        fun getLiveAndVODStreams(queryResult: Array<String>?, channel: String?): List<Movie> {
            if (queryResult == null || channel == null) {
                return listOf()
            }
            ...
            (0 until JSONArray(strResponse).length()).forEach {
                ...
                try {
                    ...
                    if (!userLiveJsonObject.toString().contains("\"stream\":null")) {
                        val stream = Movie()
                        val liveUrl = Utils.getLiveURL(channel)
                        stream.id = userLiveJsonObject.getJSONObject("data").getJSONObject("user")
                            .getJSONObject("broadcastSettings").getLong("id")
                        stream.title =
                            userLiveJsonObject.getJSONObject("data").getJSONObject("user")
                                .getJSONObject("broadcastSettings").getString("title")
                        stream.description =
                            userLiveJsonObject.getJSONObject("data").getJSONObject("user")
                                .getJSONObject("broadcastSettings").getString("title")
                        stream.duration = 0
                    ...
                    for (i in 0 until jsonedges.length()) {
                        val item = jsonedges.getJSONObject(i).getJSONObject("node")
                        ...
                        if (item.getString("previewThumbnailURL")
                            ...
                            stream.backgroundImageUrl =
                                "https://assets.twitch.tv/assets/mobile_iphone-526a4005c7c0760cb83f.png"
                            stream.cardImageUrl =
                                "https://assets.twitch.tv/assets/mobile_iphone-526a4005c7c0760cb83f.png"
