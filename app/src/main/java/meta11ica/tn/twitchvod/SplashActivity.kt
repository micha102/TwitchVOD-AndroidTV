    ...
    private fun initializePrefs() {
        ...
        if (sharedPrefs.getString("favourite_streamers", null)==null) {
            ...
            sharedPrefs.edit {
                putString(
                    "favourite_streamers",
                    streamerId.joinToString(separator = ",")
                )
            }
        }
        val editor = sharedPrefs.edit()
        // the first use of the shared preference will trigger its initialization
        ...
        lifecycleScope.launch {
            ...
            val answers = favStreamers?.let { Utils.makeTwitchPostRequest(it) }
            ...
            (0 until JSONArray(strResponse).length()).forEach {
                ...
                try {
                    ...
                    if (!userLiveJsonObject.toString().contains("\"stream\":null")) {
                        val liveUrl = Utils.getLiveURL(favStreamers?.get(it))
                        id.add(
